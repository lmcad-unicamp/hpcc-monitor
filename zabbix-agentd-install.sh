...
sudo sed -i "s@# UserParameter=@UserParameter=missingdrivers,if [ ! -z \"\$(lspci | grep 'NVIDIA')\" ]; then if [ ! -z \"\$(nvidia-smi 2> /dev/null)\" ]; then echo \"11\"; else echo \"10\"; fi; else  echo \"00\"; fi\nUserParameter=gpu.utilization,echo \"scale=3 ; \$(nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader,nounits | awk '{s+=\$1} END {printf \"%d\", s}') / \$(nvidia-smi --query-gpu=utilization.gpu --format=csv,noheader,nounits | awk 'END { print NR }')\" | bc@g" /etc/zabbix/zabbix_agentd.conf
